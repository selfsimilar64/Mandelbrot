...
JNICALL
Java_com_selfsimilartech_fractaleye_FractalRenderer_iterateKleinianNative(
        JNIEnv *env,
        jobject, /* this */
        jobject data) {
...
//        }
        ...
        for (int x = 0; x < width; x++) {
            ...
            complex q = csub(z1, c);
            double phi = atan2(q.y, q.x);
            z1 = cadd(c, cdiv(cexpi(phi), cabs(q)));
            ...
            for (int n = 0; n <= maxIter; n++) {
                ...
                // shape loop
                w = z1;
                w.x = fmod(z1.x + 2.0*maxIter - 1.0, 2.0) - 1.0;
                if (z1.y < p1.x/2.0) {
                    z = cdiv(csub(cmult(p1.x, w), i), cmult(cneg(i), w));
                }
                else {
                    z = cdiv(i, cadd(cmult(i, w), p1.x));
                }

                zModSqr = cabs2(z);
                //smoothSum += exp(-sqrt(zModSqr));

                if (zModSqr > escapeRadius*escapeRadius || z.y < 0.0 || z.y > p1.x) {
